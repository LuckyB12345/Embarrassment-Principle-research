...
\begin{table*}[!htbp]
\centering
\caption{Reduced table 1 ( tightened for column width )}
\setlength{\tabcolsep}{3pt} % 自动缩窄列间距
\resizebox{\textwidth}{!}{%
...
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{2.4pt} % 自动缩窄，不超宽
...
\begin{itemize}
    \item For all cuts with $z_{\rm min}\le0.09$, the oscillation frequency $\beta$ remains extremely stable, varying only between $18.18$ and $18.23$ (relative variation $<0.3\%$). This stability strongly supports the existence of an intrinsic periodic structure in the data.
    ...
    \item At $z_{\rm min}=0.10$, $\Delta\chi^2$ drops abruptly to $1.10$, and the signal becomes insignificant. The oscillation frequency $\beta$ shows a jump to $19.11$, but since the model no longer improves the fit, this jump is not statistically meaningful.
    \item For $z_{\rm min}=0.20$, $\Delta\chi^2=1.71$, confirming that the anomaly is confined to $z<0.1$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sharp localization at $z<0.1$}: Future surveys with improved low-redshift calibration should confirm that the periodic signal is sharply concentrated below $z=0.1$, as indicated by our redshift-cut tests.
