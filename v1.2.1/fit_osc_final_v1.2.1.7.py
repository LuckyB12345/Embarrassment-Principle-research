...
def alpha_of_z(x, alpha0):
    return alpha0 * np.exp(-x)  # 最安全原版，保留不动
...
def rho_osc_at_z(z, A, alpha0, b):
    def integrand_rho(x):
        ...
        exp_fac = np.exp(exp_arg)
        return 3 * A * exp_fac * np.sin(b * x) / (1.0 + x)
    
    try:
        val, err = quad(integrand_rho, 0, z, limit=INTEGRAL_LIMIT, epsrel=INTEGRAL_EPSREL)
    except:
        val = -300.0
...
def one_plus_w(z, A, alpha0, b):
    ...
    exp_arg = np.clip(-a_z * z, -700, 700)
    return A * np.exp(exp_arg) * np.sin(b * z)
...
def chi2_total(theta, z, mu, cov, bao, is_lcd):
    if is_lcd:
        ...
        # 物理约束：A ≤ 0.005，α₀ ∈ [-15, 0]
        if not (0 <= A <= 0.005 and -15 <= alpha0 <= 0 and 10 <= b <= 50 and 0.15 <= Om <= 0.35 and 67 <= H0 <= 78):
            return 1e10
...
if __name__ == "__main__":
    print("="*70)
    print("v1.7 【阿D纠错定稿】所有错误已修复 | 物理合规")
    print("✅ BAO修正 | ✅ 协方差矩阵修正 | ✅ A≤0.005 | ✅ 1+w稳定")
    print("="*70)
    ...
    init = [0.001, -8.0, 18.0, 0.25, 72.0]
    bounds = [(0, 0.005), (-15, 0), (10,50), (0.15,0.35), (67,78)]
