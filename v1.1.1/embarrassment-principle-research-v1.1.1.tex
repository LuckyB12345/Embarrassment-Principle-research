...
We perform fully independent $\chi^2$ fitting using custom Python code (fit\_osc-1.1.1.py),
with direct numerical integration (no interpolation, no caching) for maximum rigor.
The integration relative tolerance is set to $10^{-7}$, and we verified that further tightening changes $\Delta\chi^2$ by less than 0.1.
...
\begin{itemize}
    ...
    \item High-precision numerical integration (epsrel=1e-7) eliminates artifacts.
...
\subsection{Monte Carlo Test for Overfitting}
To exclude the possibility that the large $\Delta\chi^2$ arises from overfitting noise, we performed a Monte Carlo simulation under the null hypothesis that the data follow the best-fit smooth $\Lambda$CDM model (fixed $r_d$, $\Omega_m=0.0177$, $H_0=76.25$) with no oscillatory component. We generated 30 mock datasets by adding correlated noise (using the Pantheon+ covariance matrix) to the $\Lambda$CDM predictions. Each mock dataset was then fitted with both $\Lambda$CDM and the oscillatory model, and the $\Delta\chi^2$ was recorded.

The distribution of $\Delta\chi^2$ from the 30 simulations is shown in Figure~\ref{fig:montecarlo}. The simulated $\Delta\chi^2$ values are small, with a maximum of $4.89$ and a mean near zero. In stark contrast, the real data yield $\Delta\chi^2 = 147.28$, which is more than 30 times larger than the maximum observed in any simulation. The $p$-value (fraction of simulations with $\Delta\chi^2 \ge 147.28$) is $0.00$ (i.e., $<0.03$). This convincingly demonstrates that the oscillatory signal is not an artifact of overfitting but represents a genuine statistical anomaly in the data.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{montecarlo_deltachi2.png}
    \caption{Distribution of $\Delta\chi^2$ from 30 Monte Carlo simulations under the null hypothesis of smooth $\Lambda$CDM. The red dashed line indicates the value obtained from the real data ($\Delta\chi^2=147.28$). All simulated values lie below 5, far from the real signal.}
...
Monte Carlo simulations confirm that such a large $\Delta\chi^2$ never arises from pure noise: the maximum $\Delta\chi^2$ in 30 simulations was $4.89$, and the $p$-value is $<0.03$.
