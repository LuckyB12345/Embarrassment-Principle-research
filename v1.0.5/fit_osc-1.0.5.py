...
def plot_results(z, mu, best):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('fig_residual_final.png', dpi=200)
    plt.close()
    ...
    plt.ylabel('w(z)')
    plt.tight_layout()
    plt.savefig('fig_wz_final.png', dpi=200)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('fig_Hz_final.png', dpi=200)
    plt.close()
    print("✅ 图片已保存: fig_residual_final.png, fig_wz_final.png, fig_Hz_final.png")
...
if __name__ == "__main__":
    ...
    results = []
    print("\n" + "="*80)
    print("最终版：自由α模型，Ωm下界0.01，无缓存实时积分")
    print("="*80)
