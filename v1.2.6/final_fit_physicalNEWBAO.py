"""
最终完整版：DESI DR2 6点 BAO（已修正BGS红移及数据） + Pantheon+ SN + Planck 2018 联合拟合
模型：1+w(z) = A * sin(b * z) / (1+z)^2
BAO 数据来自 DESI DR2 (Abdul‑Karim et al. 2025, arXiv:2503.14738)
修正：BGS 点红移 0.20 → 0.295，观测值 3.861 → 7.391，误差 0.045 → 0.072
无 Cobaya 依赖，直接运行，输出 χ²、最佳拟合参数、Fisher 误差。
"""
...
RD_FID = 147.09           # Mpc (声视界尺度，与 DESI 官方一致)

# ========== 数据加载（SN + DESI DR2 BAO，已修正）==========
def load_data():
    # 加载 Pantheon+ SN 数据
    ...
    # DESI DR2 BAO 数据（6个点）—— 修正 BGS 点
    bao = {
        0.295: (7.391, 0.072),   # BGS (修正后)
...
if __name__ == "__main__":
    print("="*70)
    print("DESI DR2 真实 BAO (6点，已修正红移) + Pantheon+ SN + Planck 2018 联合拟合")
    ...
    # ---------- 拟合振荡模型 ----------
    print("\n>>> 拟合振荡模型 ...")
    init_osc = [1.5, 26.0, 0.15, 69.0]   # 合理的初始值
